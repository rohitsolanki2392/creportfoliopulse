...
async def list_category_files_service(user_id: str, category: str, db: AsyncSession):
    try:
        ...
        for file in files:
            file_url = "/" + (file.file_path or "").lstrip("/")
    ...
    try:
        ...
        if file_record.file_path and os.path.exists(file_record.file_path):
            try:
                ...
                logger.warning(f"Could not delete physical file {file_record.file_path}: {e}")

        await db.delete(file_record)
        await db.commit()

        return {
            "message": f"File '{file_record.original_file_name}' deleted successfully",
            "file_id": file_id
        }
